...
void HMC58X3::writeReg(unsigned char reg, unsigned char val) {
	uint8 data[2] = {reg, val};
	i2c_msg tmp;
	tmp.addr = HMC58X3_ADDR;
	tmp.flags = 0;
	tmp.length = 2;
	tmp.data =  &data[0];
...
void HMC58X3::getRaw(int16_t *x,int16_t *y,int16_t *z) {
	...
	tmp[0].addr = HMC58X3_ADDR;
	tmp[0].flags = 0;
	tmp[0].length = 1;
	uint8 cmd = HMC58X3_R_XM;
	tmp[0].data = &cmd;

	tmp[1].addr = HMC58X3_ADDR;
...
void HMC58X3::getID(char id[3]) 
{
	...
	tmp[0].flags = 0;
	tmp[0].length = 1;
	unsigned char cmd = HMC58X3_R_IDA;
	tmp[0].data = &cmd;

	tmp[1].addr = HMC58X3_ADDR;
	tmp[1].flags = I2C_MSG_READ;
	tmp[1].length = 3;
	tmp[1].data = (uint8*) &id[0];
